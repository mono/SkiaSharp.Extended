...
namespace SkiaSharp.Extended.Svg
{
	public class SKSvg
	{
		...
		{
			if (symbol == null || use == null) return;
			...
			try
			{
				...
				var useSize = ReadElementHeightAndOrWidth(use);
		...
		private void ScaleViewBoxToSize(SKCanvas canvas, SKRect viewBox, SKSize size, string aspectRatio)
		{
			...
			if (viewBox.IsEmpty || Math.Abs(viewBox.Width) == 0f || Math.Abs(viewBox.Height) == 0f) return;
			...
			if (Math.Abs(size.Width) == 0f) scaleX = scaleY;
			if (Math.Abs(size.Height) == 0f) scaleY = scaleX;

			var ratio = aspectRatio.Split(' ').FirstOrDefault();
		...
		private SKSize ReadElementHeightAndOrWidth(XElement e)
		{
			if (e == null) return SKSize.Empty;

			// I think this could be made more general.  It would be nice if there was a single function
			// do determine an element's width and height in it's parent, based on its parentage and viewboxes/ports
			float width = 0f;
			float height = 0f;
			var element = e;
		...
		{
			if (e == null) return SKPoint.Empty;
		...
		{
			if (e == null) return SKRect.Empty;

			SKRect viewBox = new SKRect();

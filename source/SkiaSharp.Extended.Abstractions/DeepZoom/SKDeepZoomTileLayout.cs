...
/// Previously named <c>SKDeepZoomTileScheduler</c>; renamed to reflect its true purpose
/// as a tile layout/geometry calculator rather than an async work scheduler.
/// </summary>
public class SKDeepZoomTileLayout
{
    /// <summary>
    ...
    /// Returns an <see cref="SKDeepZoomRectF"/> in parent-bitmap pixel coordinates.
    /// </summary>
    public SKDeepZoomRectF GetFallbackSourceRect(
        SKDeepZoomTileId requested, SKDeepZoomTileId parent, SKDeepZoomImageSource tileSource)
    {
        ...
        return new SKDeepZoomRectF(srcX, srcY, reqWidthInParent, reqHeightInParent);
    }

    /// <summary>
    /// Computes the screen-space destination rect for a tile given the current viewport.
    /// Previously on <see cref="SKDeepZoomRenderer"/>; moved here because it is pure geometry,
    /// independent of any rendering backend.
    /// </summary>
    public SKDeepZoomRectF GetTileDestRect(

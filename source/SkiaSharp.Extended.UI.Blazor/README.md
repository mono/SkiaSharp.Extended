...
- `Components/` contains reusable Blazor components.
  ...
  `SKAnimatedSurfaceView` owns Canvas/OpenGL rendering and optional frame updates; Lottie composes the animated surface.
...
`SKAnimatedSurfaceView` depends on `SkiaSharp.Views.Blazor`; `SKLottieView` additionally uses the core `SkiaSharp.Extended` project and
`SkiaSharp.Skottie`. Keep native surface and render-loop behavior in the animated surface, source lifecycle behavior in the Lottie adapter,
and playback behavior in the shared player. The animated surface shares the core monotonic frame counter with MAUI and draws an FPS overlay
only in DEBUG builds. Test observable frame and lifecycle behavior.

...
namespace SkiaSharp.Extended.DeepZoom
{
    ...
    /// </summary>
    public class SKDeepZoomImageSource
    {
        ...
        public SKDeepZoomImageSource(int imageWidth, int imageHeight, int tileSize, int overlap, string format)
        {
        ...
        /// </summary>
        public IReadOnlyList<SKDeepZoomDisplayRect> DisplayRects { get; internal set; } = Array.Empty<SKDeepZoomDisplayRect>();
        ...
        public static SKDeepZoomImageSource Parse(string xml)
        {
        ...
        public static SKDeepZoomImageSource Parse(Stream stream)
        {
        ...
        public static SKDeepZoomImageSource Parse(Stream stream, string? baseUri)
        {
            var doc = XDocument.Load(stream);
        ...
        public static SKDeepZoomImageSource Parse(string xml, string? baseUri)
        {
            var doc = XDocument.Parse(xml);
            return ParseDocument(doc, baseUri);
        }

        private static SKDeepZoomImageSource ParseDocument(XDocument doc, string? baseUri)
        {
            ...
            var source = new SKDeepZoomImageSource(width, height, tileSize, overlap, format);
            ...
            {
                var rects = new List<SKDeepZoomDisplayRect>();
                foreach (var drElement in displayRectsElement.Elements(ns + "DisplayRect"))
                {
                    ...
                    if (rectElement != null)
                    {
                        ...
                        rects.Add(new SKDeepZoomDisplayRect(rx, ry, rw, rh, minLevel, maxLevel));
                    }
                }

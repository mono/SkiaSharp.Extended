...
For a fully custom loader, derive from `SKLottieImageSource` and override `LoadAnimationAsync`. The view supplies the registered default
`HttpClient` and cancellation token. Return a new `SKLottieAnimation` result for each successful load; native Skottie animations contain
mutable playback state and must not be shared between views. Assign a new custom source instance when its underlying data should be
reloaded.

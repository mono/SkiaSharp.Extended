...
> monotonic timing and rolling frame-rate calculation; this diagnostic is not
...
// From a stream factory. Return a fresh stream for each reload.
lottieView.Source = new SKStreamLottieImageSource { Stream = _ => OpenAnimationStreamAsync() };
```
    ...
    <skia:SKLottieView Source="animation.json">
        <skia:SKLottieView.ControlTemplate>
            <ControlTemplate>
                ...
                <views:SKGLView x:Name="PART_DrawingSurface" />
            </ControlTemplate>
        </skia:SKLottieView.ControlTemplate>
    </skia:SKLottieView>
</ContentPage>

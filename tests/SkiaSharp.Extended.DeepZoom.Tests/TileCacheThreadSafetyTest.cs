...
public class TileCacheThreadSafetyTest
{
    ...
    public async Task ConcurrentGetAndPut_DoesNotCorrupt()
    {
        ...
        for (int i = 0; i < 50; i++)
        {
            ...
            tasks.Add(Task.Run(() =>
            {
                cache.Put(new SKDeepZoomTileId(50 + localI, 0, 0), new SKDeepZoomBitmapTile(new SKBitmap(10, 10)));
            }));
        }
    ...
    public async Task ConcurrentClearAndPut_DoesNotCorrupt()
    {
        ...
        for (int i = 0; i < 20; i++)
        {
            ...
            {
                for (int j = 0; j < 10; j++)
                    cache.Put(new SKDeepZoomTileId(localI * 10 + j, 0, 0), new SKDeepZoomBitmapTile(new SKBitmap(10, 10)));
            }));
        }
    ...
    public async Task ConcurrentDisposeAndPut_NoExceptionsOrLeaks()
    {
        ...
        cache.Put(new SKDeepZoomTileId(9999, 0, 0), new SKDeepZoomBitmapTile(new SKBitmap(10, 10)));
    ...
    public async Task PutDuringDispose_ItemsAreCleanedUp()
    {
        ...
        var putTask = Task.Run(() =>
        {
            for (int i = 0; i < 100; i++)
            {
                ...
                {
                    var bmp = new SKBitmap(10, 10);
                    cache.Put(new SKDeepZoomTileId(100 + i, 0, 0), new SKDeepZoomBitmapTile(bmp));
                }
            ...
            for (int threadId = 0; threadId < 3; threadId++)
            {
                ...
                tasks.Add(Task.Run(() =>
                {
                    try
                    {
                        ...
                        {
                            var bmp = new SKBitmap(10, 10);
                            cache.Put(new SKDeepZoomTileId(localThreadId * 200 + i, 0, 0), new SKDeepZoomBitmapTile(bmp));
                        }
                    }
                    catch (Exception ex)
                    {

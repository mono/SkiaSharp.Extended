...
public class SKLottiePlayerTest
{
	...
	[Fact]
	public void UpdateAfterCompletion_IsNoOpUntilSeek()
	{
		using var anim = CreateAnimation();
		var player = new SKLottiePlayer();
		player.Animation = anim;
	...
	[Fact]
	public void Seek_ToBoundary_DoesNotRestartOrConsumeRepeats()
	{
		using var anim = CreateAnimation();
		var player = new SKLottiePlayer();
		player.Animation = anim;
		...
		// The next update consumes the first repeat; seeking itself did not.
		player.Update(TimeSpan.FromSeconds(0.1));
		Assert.Equal(TimeSpan.FromSeconds(0.1), player.Progress);
		Assert.False(player.IsComplete);
	}
	...
	[Fact]
	public void RepeatRestart_PreservesOvershootAcrossBoundaries()
	{
		using var anim = CreateAnimation();
		var player = new SKLottiePlayer
		{
	...
	[Fact]
	public void RepeatReverse_PreservesOvershootAcrossBoundaries()
	{
		using var anim = CreateAnimation();
		var player = new SKLottiePlayer
		{
	...
	[Fact]
	public void Seek_ClearsCompletionWithoutRaisingCompletion()
	{
		using var anim = CreateAnimation();
		var player = new SKLottiePlayer();
		player.Animation = anim;
		...
		var updated = 0;
		player.AnimationCompleted += (_, _) => completed++;
		player.AnimationUpdated += (_, _) => updated++;
	...
	public void SwitchingFromReverseToRestart_WithNegativeSpeed_DoesNotStick()
	{
		...
		player.Update(player.Duration); // reaches Zero → flip

...
public class TileCacheThreadSafetyTest
{
    ...
    public async Task ConcurrentGetAndPut_DoesNotCorrupt()
    {
        ...
        for (int i = 0; i < 50; i++)
        {
            ...
            tasks.Add(Task.Run(() =>
            {
                cache.Put(new SKImagePyramidTileId(50 + localI, 0, 0), new SKImagePyramidTile(SKImage.Create(new SKImageInfo(10, 10))));
            }));
        }
    ...
    public async Task ConcurrentClearAndPut_DoesNotCorrupt()
    {
        ...
        for (int i = 0; i < 20; i++)
        {
            ...
            {
                for (int j = 0; j < 10; j++)
                    cache.Put(new SKImagePyramidTileId(localI * 10 + j, 0, 0), new SKImagePyramidTile(SKImage.Create(new SKImageInfo(10, 10))));
            }));
        }
    ...
    public async Task ConcurrentDisposeAndPut_NoExceptionsOrLeaks()
    {
            ...
            tasks.Add(Task.Run(() =>
            {
                try
                {
                    for (int i = 0; i < 1000; i++)
                    {
                        ...
                        cache.Put(tileId, new SKImagePyramidTile(SKImage.FromBitmap(bmp)));
                    }
                }
                catch (Exception ex)
                {
        ...
        cache.Put(new SKImagePyramidTileId(9999, 0, 0), new SKImagePyramidTile(SKImage.Create(new SKImageInfo(10, 10))));
    ...
    public async Task PutDuringDispose_ItemsAreCleanedUp()
    {
        ...
        var putTask = Task.Run(() =>
        {
            for (int i = 0; i < 100; i++)
            {
                ...
                {
                    var bmp = new SKBitmap(10, 10);
                    cache.Put(new SKImagePyramidTileId(100 + i, 0, 0), new SKImagePyramidTile(SKImage.FromBitmap(bmp)));
                }
            ...
            for (int threadId = 0; threadId < 3; threadId++)
            {
                ...
                tasks.Add(Task.Run(() =>
                {
                    try
                    {
                        ...
                        {
                            var bmp = new SKBitmap(10, 10);
                            cache.Put(new SKImagePyramidTileId(localThreadId * 200 + i, 0, 0), new SKImagePyramidTile(SKImage.FromBitmap(bmp)));
                        }
                    }
                    catch (Exception ex)
                    {
